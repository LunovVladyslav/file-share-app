...
- **Роутер не потрібен** — потрібна лише спільна мережа, і точка доступу з телефона нею є. Ніщо не йде через мобільний трафік і ніщо не йде через інтернет.
...
Обидва пристрої мають бути в одній мережі. Це може бути ваш звичайний Wi-Fi
або точка доступу з будь-якого телефона — мобільні дані можна лишити
вимкненими, бо те, що два пристрої на точці передають одне одному, її не
покидає.
...
<tr>
<td width="50%"><img src="docs/screenshots/transfer-detail.png" alt="Передача файл за файлом"><br><em>Будь-яка передача файл за файлом — і куди потрапив кожен</em></td>
<td width="50%"><img src="docs/screenshots/advanced.png" alt="Пряме підключення та гайд"><br><em>Немає мережі? Кроки залежать від того, які це два пристрої</em></td>
</tr>
<tr>
...
Список переживає застосунок. Останні п'ятдесят передач зберігаються на диску —
з ким, скільки тривала, куди потрапили файли, — тож закрите вікно більше не
стирає запис. Натисніть будь-яку, щоб побачити її файл за файлом: що вже
прийшло, що передається просто зараз, що ще в черзі, і як показати в провіднику
один потрібний файл із чотирьохсот.
...
права 600), `peers.json` (закріплені ключі) і `history.json` (останні п'ятдесят
передач — лише підсумки, ніколи не список імен файлів). Змінна `FLYSHARE_HOME`
переносить усе в іншу теку — саме це робить можливими портативний запуск і два
інстанси поруч.

Порти змінюються через `FLYSHARE_DISCOVERY_PORT`, `FLYSHARE_TRANSFER_PORT`,
`FLYSHARE_UI_PORT`.

Інтерфейс відкривається окремим вікном — будь-який браузер на Chromium уміє
показати сторінку як самостійне вікно, тож немає ні адресного рядка, ні
вкладок, а на панелі задач з'являється власний запис. Прапорець `--no-app`
повертає звичайну вкладку, а `--no-open` не відкриває нічого — адреса
надрукована при запуску.
...
Якщо ніщо з цього не допомогло — мережа відкидає оголошення, за якими пристрої
знаходять одне одного, але звичайний трафік пропускає; так поводяться гостьові
мережі та деякі точки доступу. **Налаштування → Додатково → Пряме підключення**
бере адресу, показану вгорі на іншому екрані, і стукає прямо в неї. Достатньо
однієї адреси на одному пристрої — далі вони бачать одне одного.

Та сама вкладка відповідає й на другу половину питання — що робити, коли мережі
немає взагалі. Оберіть дві платформи, і вона розкладе кроки: точка доступу з
телефона, якщо один із них телефон (мобільні дані можна лишити вимкненими —
те, що пристрої передають одне одному, не покидає точки доступу), кабель
Ethernet між двома комп'ютерами або будь-який телефон, що тримає мережу для
обох. Windows і macOS зазвичай не піднімуть точку доступу без інтернет-з'єднання,
яким ділитися, а саме його в цій ситуації й немає; Linux — підніме.
...
- Передачу можна поставити на паузу й продовжити, поки обидва застосунки відкриті; перервану — застосунок закрито, мережа зникла — продовжити не можна, вона починається спочатку.
...
- Скидання з десятками тисяч файлів передається списком, розбитим на кілька кадрів. Пристрій на старішій збірці — зокрема застосунок на Android — такого списку не прочитає й отримає відмову до початку передачі, а не обрив зв'язку посеред неї.
